...
let ApartmentsService = ApartmentsService_1 = class ApartmentsService {
    ...
    async update(request, user, apartmentId, payload) {
        ...
        if (typeof payload.selfManagement === 'boolean')
            updateData.selfManagement = payload.selfManagement;
    ...
    async remove(request, user, apartmentId) {
        ...
        if (context) {
            try {
                ...
                this.logger.error(`Failed to delete apartment storage folder ${context.path}`, error instanceof Error ? error.stack : String(error));
            }
        }
        ...
        try {
            ...
            if (!pendingInvitations.empty) {
                await this.apartmentsRepository.commitInChunks(pendingInvitations.docs.map((document) => (batch) => {
            ...
            this.logger.error(`Failed to revoke pending invitations for apartment ${apartmentId}`, error instanceof Error ? error.stack : String(error));
        }
        await ref.delete();
        if (buildingId) {
