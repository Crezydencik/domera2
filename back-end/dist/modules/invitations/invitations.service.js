...
let InvitationsService = class InvitationsService {
    ...
    async revoke(request, user, invitationId) {
        ...
        const email = typeof data.email === 'string' ? (0, invitation_token_1.normalizeEmail)(data.email) : '';
        await ref.set({
            status: 'revoked',
            revokedAt: new Date(),
        }, { merge: true });
    ...
    async clearPendingApartmentInvite(apartmentId, inviteType, email, invitationId) {
        ...
        if (inviteType === 'owner') {
            ...
            if (!matchesOwner || ownerActivated)
                return;
            await ref.set({
                ownerEmail: null,
                ownerId: null,
        ...
        const nextTenants = tenants.filter((tenant) => {
            if (!tenant || typeof tenant !== 'object')
                return true;
            const tenantEmail = typeof tenant.email === 'string' ? (0, invitation_token_1.normalizeEmail)(tenant.email) : '';
        ...
        if (nextTenants.length === tenants.length)
            return;
        await ref.set({
            tenants: nextTenants,
