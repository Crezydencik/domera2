...
export class InvoicesService {
  ...
  async listUploadHistory(user: RequestUser, query: Record<string, string | undefined>) {
    ...
    const rawItems: Array<Record<string, unknown>> = await Promise.all(
      snapshots
  ...
  async list(user: RequestUser, query: Record<string, string | undefined>) {
    ...
    const items = snapshots.flatMap(({ apartmentId, snapshot }) =>
      snapshot.docs.map((doc) => this.invoiceItemFromDoc(doc, apartmentId)),
    );
